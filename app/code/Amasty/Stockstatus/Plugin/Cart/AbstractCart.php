...
class AbstractCart
{
    ...
    public function aroundGetItemHtml(
        ...
        if ($this->helper->getModuleConfig('display/display_in_cart') && $item->getProduct()->getData('sku')) {
            $find = '</strong>';
            $product = $item->getProduct();
            $status = $this->helper->getProductStockStatus($product, $item);

            if ($status) {
                $status = '<div class="amstockstatus-cart">' . $status . $this->helper->getInfoBlock() . '</div>';
