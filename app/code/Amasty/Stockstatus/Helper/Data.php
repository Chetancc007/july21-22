...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     * @return string
     */
    public function getStatusIconImage($status, $product = null)
    {
        ...
        if ($iconUrl) {
            ...
            $iconUrl = sprintf(
                '<img src="%s" class="amstockstatus_icon" alt="%2$s" title="%2$s">',
                $iconUrl,
                $status
            );
        }
    ...
    private function getCustomStockStatus(Product $product)
    {
        ...
        if ($status) {
            ...
            $imageHtml = $this->getStatusIconImage($status, $product);
