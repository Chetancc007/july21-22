...
class Rate extends AbstractModel
{
    ...
    {
        $this->_init('Amasty\StorePickup\Model\ResourceModel\Rate');
    }
    ...
    public function deleteBy($methodId)
    {
         ...
         */
        $resource = $this->_objectManager->get('Amasty\StorePickup\Model\ResourceModel\Rate');
    ...
    public function findBy(RateRequest $request, Collection $collection)
    {
            ...
            foreach ($intersectTypes[$key] as $shippingType) {
                ...
                if ($allTotals['qty'] > 0
                    ...
                    foreach ($this->calculateCosts($ratesData, $totals, $request, $shippingType) as $key => $cost) {
                         ...
                         */
                        $method = $this->_objectManager->create('Amasty\StorePickup\Model\Method')->load($key);
        ...
        $method = $this->_objectManager->create('Amasty\StorePickup\Model\Method');
