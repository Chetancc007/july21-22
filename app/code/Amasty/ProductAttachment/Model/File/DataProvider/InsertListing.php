...
class InsertListing extends \Magento\Ui\DataProvider\AbstractDataProvider
{
    ...
    public function addFilter(\Magento\Framework\Api\Filter $filter)
    {
        ...
        if ($filter->getField() == FileInterface::FILE_ID && $filter->getConditionType() == 'nin') {
            ...
            $condition = '(' . CreateFileScopeTables::FILE_STORE_TABLE_NAME . '_store.' . $filter->getField()
                . ' IS NOT NULL'
                . ' AND ' . CreateFileScopeTables::FILE_STORE_TABLE_NAME . '_store.' . $filter->getField()
                . ' ' . $filter->getConditionType() . ' ?)'
                . ' OR (' . CreateFileScopeTables::FILE_STORE_TABLE_NAME . '_store.' . $filter->getField() . ' IS NULL'
                . ' AND ' . CreateFileScopeTables::FILE_STORE_TABLE_NAME . '.' . $filter->getField() . ' '
                . $filter->getConditionType() . ' ?)';
